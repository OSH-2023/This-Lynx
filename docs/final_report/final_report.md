...
#### 实现跨平台的零成本移植
vega在分布式运行时不需要在从机上下载环境，但是限于配置文件和平台依赖的库函数使用，这种移植还未进行实现和测试。但是利用Rust的条件编译`#[cfg(target os="windows")]`可以编写两套平台下的函数，以及在Context,env模块中编写解耦的配置加载逻辑理论上就可以实现跨平台的移植。Spark运行分布式时需要在从机上安装应用，vega做好移植后不需要安装，只要网络配置正常就可以进行测试。因此我们可以考虑在未来工作中实现跨平台的零成本移植。
