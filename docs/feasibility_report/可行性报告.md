...
Spark Streaming的数据抽象为DStream类型。DStream代表一个连续的数据流，它本质上是一个RDD序列。
#### 源码分析
...
**DStream**
DStream意为离散化数据流，是用于封装流式数据的数据结构，其内部由一系列连续的RDDs构成，每个RDD代表特定时间间隔内的一批数据。这个类是一个抽象类，具体实现由继承它的具体的DStream（如InputDStream）完成。
其形式如下：
```scala
...
**JobScheduler**
JobScheduler负责调度在Spark上执行的作业，它使用JobGenerator生成作业并使用线程池执行。
其形式如下：
```scala
